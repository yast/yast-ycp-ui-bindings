...
module Yast
  class SlideShowDemoClient < Client
    def main
      Yast.import "UI"
      ...
      while true
        ...
        if @button == :back
          ...
          @delta = 100
        elsif @button == :showSlide
          Builtins.y2milestone("Switching to slide show")
          UI.ReplaceWidget(:tabContents, @slideShowPage)
        elsif @button == :showDetails
          Builtins.y2milestone("Switching to details")
          # y2debug( "detailsPage:\n%1", detailsPage );
...
Yast::SlideShowDemoClient.new.main
